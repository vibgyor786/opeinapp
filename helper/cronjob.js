...
async function cron() {
  ...
  try {
    ...
    for (let i = 0; i < data.length; i++) {
      ...
      const usertaskdata = await task.find({
        userId: data[i]._id,
        isdeleted: false,
      ...
      const usertaskdatall = await task.find({
        userId: data[i]._id,
        isdeleted: false,
      });
      ...
      if (usertaskdata.length > 0 && usertaskdatall.length > 0) {
        console.log(
          `changing priority for user having phone no. : ${data[i].phoneNumber}`
        );
        ...
        const x = await user.findByIdAndUpdate(data[i]._id, {
          $set: { priority: p },
        });
        p = p + 1;
      }
